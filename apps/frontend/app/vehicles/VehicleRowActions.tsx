...
import { deleteVehicle, updateVehicle, assignRfidTag } from "./actions";
...
export default function VehicleRowActions({ vehicle }: { vehicle: Vehicle }) {
  ...
  const [rfidInput, setRfidInput] = useState(vehicle.rfidTag ?? "");
  ...
  const handleAssignRfid = async (e: React.FormEvent) => {
    ...
    setRfidError("");
    setLoading(true);
    const result = await assignRfidTag(
      vehicle.id,
      rfidInput.trim() || null
    );
    if (result.success) {
      ...
      setRfidError(result.error ?? "Error desconocido");
    }
    setLoading(false);
  };
  ...
  return (
    <>
      <div className="flex justify-end gap-1">
        <button
          onClick={() => { setIsRfidOpen(true); setRfidError(""); setRfidSuccess(false); setRfidInput(vehicle.rfidTag ?? ""); }}
          className="p-2 hover:bg-[var(--color-tertiary-container)] rounded-lg transition-colors text-[var(--color-on-surface-variant)] hover:text-[var(--color-tertiary)]"
      ...
      {/* RFID Tag Assignment Modal */}
      {isRfidOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-black/40 backdrop-blur-sm"
            onClick={() => !loading && setIsRfidOpen(false)}
          />
          <div className="relative bg-[var(--color-surface-container-lowest)] w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden border border-[var(--color-outline-variant)] animate-in fade-in zoom-in duration-200">
            {/* Header */}
            <div className="bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-tertiary)] px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-white">nfc</span>
                <h3 className="text-white font-black text-sm tracking-wider">Asignar TAG RFID</h3>
              </div>
              <button onClick={() => setIsRfidOpen(false)} className="text-white/70 hover:text-white transition-colors">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>

            <form onSubmit={handleAssignRfid} className="p-6 space-y-5">
              <div>
                <p className="text-xs text-[var(--color-on-surface-variant)] mb-1">
                  Vehículo: <span className="font-mono font-bold text-[var(--color-on-surface)]">{vehicle.plate}</span>
                </p>
                <p className="text-[0.7rem] text-[var(--color-on-surface-variant)] leading-relaxed">
                  Pega aquí el UID leído por el RC522 (ej:{" "}
                  <span className="font-mono">A3:F2:01:CC</span>). Déjalo vacío para desasociar el TAG.
                </p>
              </div>

              {/* Current tag indicator */}
              {vehicle.rfidTag && (
                <div className="flex items-center gap-2 bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/20 rounded-lg px-3 py-2">
                  <span className="material-symbols-outlined text-sm text-[var(--color-primary)]">nfc</span>
                  <span className="text-[0.7rem] text-[var(--color-on-surface-variant)]">TAG actual:</span>
                  <span className="font-mono text-xs font-bold text-[var(--color-primary)]">{vehicle.rfidTag}</span>
                </div>
              )}

              <div className="space-y-1.5">
                <label className="text-[0.7rem] font-bold uppercase tracking-wider text-[var(--color-on-surface-variant)]">UID del TAG</label>
                <input
                  type="text"
                  value={rfidInput}
                  onChange={(e) => { setRfidInput(e.target.value.toUpperCase()); setRfidError(""); }}
                  placeholder="Ej: A3:F2:01:CC"
                  className="w-full bg-[var(--color-surface-container-high)] border border-[var(--color-outline-variant)] rounded-xl py-2.5 px-3 text-sm font-mono tracking-widest focus:ring-2 focus:ring-[var(--color-primary)] outline-none transition-all uppercase"
                  disabled={loading || rfidSuccess}
                />
              </div>

              {rfidError && (
                <div className="flex items-center gap-2 text-[var(--color-error)] bg-[var(--color-error-container)] rounded-lg px-3 py-2">
                  <span className="material-symbols-outlined text-sm">error</span>
                  <p className="text-xs font-semibold">{rfidError}</p>
                </div>
              )}

              {rfidSuccess && (
                <div className="flex items-center gap-2 text-[var(--color-primary)] bg-[var(--color-primary)]/10 rounded-lg px-3 py-2">
                  <span className="material-symbols-outlined text-sm">check_circle</span>
                  <p className="text-xs font-bold">TAG asignado correctamente</p>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                <button
                  ...
                  disabled={loading}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--color-outline-variant)] text-[var(--color-on-surface-variant)] font-bold text-sm hover:bg-[var(--color-surface-container-low)] transition-all disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={loading || rfidSuccess}
                  className="flex-1 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-tertiary)] text-white font-bold text-sm py-2.5 rounded-xl shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {loading && <span className="material-symbols-outlined text-sm animate-spin">sync</span>}
                  {loading ? "Guardando..." : rfidSuccess ? "✓ Guardado" : "Guardar TAG"}
                </button>
              </div>
            </form>
          </div>
